...
import { getFileContentAndSha, getBlobContentAndSha, getFile, updateFile, deleteFile } from './github';
...
export const getRecordPath = (dbName, tableName, primaryKeyVal) => `${localStorage.getItem(constants.LS_KEY_GITHUB_REPO_PATH)}/${dbName}/${tableName}/${getRecordFileName(primaryKeyVal)}`;
/**
 * @param {string} dbName
 * @param {string} tableName
...
export const getDataPath = (dbName, tableName) => `${localStorage.getItem(constants.LS_KEY_GITHUB_REPO_PATH)}/${dbName}/${getDataFileName(tableName)}`;
/**
* @param {string} dbName
* @param {string} tableName
...
export const getDataUrl = (dbName, tableName) => getGitHubFullPath(getDataPath(dbName, tableName));
/**
 * @return {string} e.g. "dbs/dbName/columns.json"
 */

const getDbTableColDefPath = dbName => `${localStorage.getItem(constants.LS_KEY_GITHUB_REPO_PATH)}/${dbName}/columns.json`;

export const getDbTablesSchemaAsync = async dbName => {
  const {
    content
  } = await getFileContentAndSha(getDbTableColDefPath(dbName));
  return content;
};
/**
...
export const getTableRows = async (dbName, tableName, signal) => {
  if (isLargeTable(dbName, tableName)) {
    const files = await getFile(`${localStorage.getItem(constants.LS_KEY_GITHUB_REPO_PATH)}/${dbName}`, signal);
    let sha;
    files.forEach(file => {
...
export const getRecordFileContentAndSha = (dbName, tableName, primaryKeyVal, signal) => {
...
export const updateRecordFile = async (dbName, tableName, primaryKey, record, sha) => {
...
export const deleteRecordFile = async (dbName, tableName, primaryKeyVal, sha) => {
  const path = getRecordPath(dbName, tableName, primaryKeyVal);
