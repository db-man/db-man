...
export const getFile = (path, signal) => octokit.request('GET /repos/{owner}/{repo}/contents/{path}', {
  owner: localStorage.getItem(constants.LS_KEY_GITHUB_OWNER),
  repo: localStorage.getItem(constants.LS_KEY_GITHUB_REPO_NAME),
  path,
  request: {
  ...
  switch (err.status) {
    case 404:
      throw new Error(`Failed to get file: file not found, file path: ${path}`, {
        cause: err
      });

    case 403:
      throw new Error(`Failed to get file: file too large, file path: ${path}`, {
        cause: err
      });
...
export const getFileV2 = (path, signal) => octokit.request('GET /repos/{owner}/{repo}/contents/{path}', {
  owner: localStorage.getItem(constants.LS_KEY_GITHUB_OWNER),
  repo: localStorage.getItem(constants.LS_KEY_GITHUB_REPO_NAME),
  path,
  request: {
...
export const getFileContentAndSha = (path, signal) => getFile(path, signal).then(({
  content,
  sha
}) => {
  let rows = [];
